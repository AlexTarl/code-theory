...
from make_code import LinearCode
...
def coder(args):
    ...
    print("Encoded message whith errors:\t{}".format('|'.join(['{0:>0{width}b}'.format(m, width=linear_code._n) for m in encoded_message])))
    print("Error for blocks:\t{}".format('|'.join(['{0:>0{width}b}'.format(e, width=linear_code._n) for e in error])))
...
def decoder(args):
    ...
    print("Decoded message whith errors:\t{}".format('|'.join(['{0:>0{width}b}'.format(m, width=linear_code._k) for m in decoded_message])))
    print("Error for blocks:\t{}".format('|'.join(['{0:>0{width}b}'.format(e, width=linear_code._n) for e in error])))
