...
class LinearCode():
    ...
    def __init__(self, n, k, d, channel_error_prob=0.005):
        if (n - k <= 0) or (n - k >= n) or (d <= 0) or ((d - 1) // 2 > n):
            raise ValueError('Must be: r,n,t > 0; n > r; n >= t')
        if not functions.gvb(n, k, d):
            raise ValueError('There is no code with this parameters r,n,t. (n={}, k={}, d={})'.format(n, k, d))
        self._n = n
        self._k = k
        self._r = self._n - self._k
        self._d = d
        self._p = channel_error_prob
        self.decode_error_prob = functions.error_probability(self._p, self._n, self._d)


    def make_A_matrix(self):
        A_matrix_t = []
        linear_comb = functions.get_combinations(self._d, self._r)
        illegal_vectors = set(map(functions.reduce_xor, linear_comb))
        ...
        for i in range(self._k):
            ...
            if (self._d - 1 > 2):
                functions.update_combinations(new_vector, illegal_vectors)
            illegal_vectors.add(new_vector)

        # A_matrix_t = [5, 6, 7, 3]
        self._A_matrix = functions.transpose_matrix(A_matrix_t, self._r)
    ...
    def make_S_vector(self):
        ...
        for i in range(1 << self._n):
            w_i = functions.hamming_weight(i)
            if w_i <= (self._d - 1) // 2:
    ...
    def __get_codeword(self, message):
        ...
        return message << self._r | functions.to_decimal(list(map(functions.binary_mult, list(zip_vec))))


    def __get_syndrome(self, y):
        zip_vec = (zip_longest(self._H_matrix_t, [y], fillvalue=y))
        return functions.to_decimal(list(map(functions.binary_mult, list(zip_vec))))
    ...
    def coder(self, message, error):
        ...
        if error is None:
            errors = functions.random_error(self._n, (self._d - 1) // 2, len(message_to_encode))
        else:
            if len(error) > self._n:
                raise ValueError('Lenght of error vector must be n bites or shortly')
    ...
    def __repr__(self):
        A_matrix_t = functions.transpose_matrix(self._A_matrix, self._k)
